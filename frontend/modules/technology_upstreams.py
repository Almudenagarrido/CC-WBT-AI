...
class TechnologyUpstreams:
    def __init__(self, excel_editor, country, subsection, model, fuel):
        ...
        self.upstream_config = self._get_upstream_config()
        self.df = None
        self.edited_df = None
    ...
    def _get_upstream_config(self):
        ...
        if self.country in upstreams:
            country_upstreams = upstreams[self.country]
            if self.model in country_upstreams:
                model_upstreams = country_upstreams[self.model]
                if self.fuel in model_upstreams:
                    return model_upstreams[self.fuel]
    ...
    def _create_df_from_values(self, values):
        ...
        df.insert(0, "Upstream", [self.fuel])
        
        return df
    
    def _ask_for_upstream_method(self):

        st.subheader(f"Upstream Configuration for {self.fuel}")
        ...
        method = st.radio(
            "How would you like to define the upstream values?",
            ...
            key=f"upstream_method_{self.country}_{self.model}_{self.fuel}"
        )
    ...
    def _get_initial_value_input(self):
        ...
        st.write("### Initial Upstream Value")

        initial_value = st.number_input(
            "Enter initial upstream value:",
            min_value=0.0,
            value=0.0,
            step=0.01,
    ...
    def _save_upstream_data(self):
        ...
        if self.edited_df is not None:
            ...
            if self.country not in self.config["UPSTREAMS"]:
                self.config["UPSTREAMS"][self.country] = {}
            
            if self.model not in self.config["UPSTREAMS"][self.country]:
                self.config["UPSTREAMS"][self.country][self.model] = {}
                
            self.config["UPSTREAMS"][self.country][self.model][self.fuel] = {year: value for year, value in values.items()}
            
            st.session_state["upstream_values"] = None
            self.save_config(self.config)
    
    def _reset_upstream_data(self):
        ...
        if self.country in upstreams:
            ...
            if self.model in country_upstreams:
                model_upstreams = country_upstreams[self.model]
                if self.fuel in model_upstreams:
                    del self.config["UPSTREAMS"][self.country][self.model][self.fuel]
                    self.save_config(self.config)
        
        st.session_state["upstream_values"] = None
        return None
    
    def _show_upstream_editor(self, df):
        
        st.subheader(f"{self.fuel} Upstream Editor")
    ...
    def __call__(self):
        ...
        existing_values = self._get_upstream_config()

        if existing_values is None:
            st.info(f"No upstream configuration found for {self.fuel}. Please set it up.")

            method = self._ask_for_upstream_method()
            values = st.session_state.get("upstream_values")
            ...
            if method == "initial_inflation":
                ...
                if initial_value is not None:
                    ...
                    st.session_state["upstream_values"] = values
        ...
        self._show_upstream_editor(self.df)
